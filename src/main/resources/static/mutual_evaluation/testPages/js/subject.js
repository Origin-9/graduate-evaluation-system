...
window.onload = () => {
    $.ajax({
        url: "/system/mutual_evaluation/inter-group_list",
        async: false,
        dataType: 'json',
        crossDomain: true,
        success: (data) => {
...
`;
        ...
        for (let j = 0; j < questionNum; j++) {
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control a_${j + 1}" placeholder="请输入评分（0-100）" min="1" max="100" step="1" value="66" required>
            </div>
        </div>
            `;
        }
...
let sendJson = json => {
    $.ajax({
        url: "/system/mutual_evaluation/addGroupScore",
        async: false,
        type: 'PUT',
        dataType: 'json',
